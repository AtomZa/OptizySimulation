...
for lamb = 1:66
lambda = lamb*0.01+0.34;
disp("Iteration " + lamb +" (" + lambda + " um)")
tic
...
zarr = 0:50:110000; %array of propagation length
...
nn = 1:length(nsolution);
mm = 1:length(volfact);
        nmedium = (volfact(mm)*nzno) + ((1-volfact(mm))*nsolution(nn));
...
qq = 1:length(nsolution);
nn = 1:length(volfact);
        if ncladdingmmf >= ncoremmf
            return
        end
...
qq = 1:length(neffcellmmf);
j = 1:length(neffcellmmf);
...
etacell = cell(size(neffcellmmf));
    ...
    qq = 1:length(neffcellmmf);
        ...
        nn = 1:length(zarr);
        mm = 1:length(tempneffmmf);
                beth = tempbet(mm);
                ahb = tempajbar(mm);
                pp = 1:length(tempneffmmf);
                    ajb = tempajbar(pp);
                    betj = tempbet(pp);
                    %coup1  = (ajb.^2).*((conj(ahb)).^2);
                    coup10 = ones(length(mm), length(mm));
                    coup10 = ((ajb.^2).').*coup10;
                    coup11 = ones(length(mm), length(mm));
                    coup11 = ((conj(ahb)).^2).*coup11;
                    coup12 = coup10*coup11;
                    coup12 = reshape(coup12, [], 1);
                    
                    coup20 = ones(length(mm), length(mm));
                    coup20 = (betj.').*coup20;
                    coup21 = ones(length(mm), length(mm));
                    coup21 = (beth).*coup21;
                    coup22 = coup20 - coup21;
                    coup22 = reshape(coup22, [], 1);
                    coup23 = coup22*zarr(nn);
                    coup24 = exp(1j.*(coup23));

                    coup = (coup12.')*coup24;
                    etaarr(1,nn) = etaarr(1,nn)+coup;
...
nn = 1:length(neffcellmmf);
        ...
        hold on
        bruh = 10.*log(cell2mat(etacell(nn,mm)))./log(10);
        realbruh = real(bruh);
        %plot(zarr, realbruh);
        %xlabel('Porpagation Distance (micron)')
        %ylabel('Coupling Efficiency (dB)');
        
        bigbruh = cell2mat(etacell(nn,mm));
        super(lamb,:) = bigbruh;
toc
end
figure
surf(lam,zarr,real(super).',"EdgeColor","None");
